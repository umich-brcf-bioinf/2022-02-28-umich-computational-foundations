...
Enter your user credentials and click <kbd>Sign In</kbd>. The credentials were provided via email, but if you forget yours, a helper can retrieve it for you.
...
2. In this window, select **Existing Directory**. For "Project working directory", click <kbd>Browse...</kbd>, select the "CF_R" folder, and click <kbd>Choose</kbd>. This will use the `/home/user/CF_R` folder as the project directory.
...
Note there are already some folders in this directory to help us organize our work.

- `data/` is where any input data will go.
- `docs/` is where any documentation will go.
- `results/` is where figures, tables, and processed data will go.
- `scripts/` is where files containing our analysis code will go.
...
Click the <kbd>File</kbd> menu and select <kbd>New File</kbd> and then <kbd>R Script</kbd>. Before we go any further, save your script by clicking the save/disk icon that is in the bar above the first line in the script editor, or click the <kbd>File</kbd> menu and select <kbd>Save</kbd>. In the "Save File" window that opens, name your file **"r_basics"** in the "File name" field, and double click the `scripts` folder to place the new R script there. The new script **r_basics.R** is now in the `scripts` folder. You can see that by clicking the `scripts` folder in the "Files" pane. And you can go back up to the main project folder by clicking the `..` to the right of the up arrow in the "Files" pane. By convention, R scripts end with the file extension **.R**.
...
To execute this command, make sure your cursor is on the same line the command is written. Then click the <KBD>Run</KBD> button that is just above the first line of your script in the header of the Source pane.
...
For the purposes of this exercise we want you to be in the directory `"/home/user/CF_R"`. What if you weren't? You can set your home directory using the `setwd()` command. Enter this command in your script, but *don't run* this yet.
...
> ## Tip: Never use `setwd()`
>
> Wait, what was the last 2 minutes about? Well, setting your working directory is something you need to do, you need to be very careful about using this as a step in your script. For example, what if your script is being run on a computer that has a different directory structure? The top-level path in a Unix file system is root `/`, but on Windows it is likely `C:\`. This is one of several ways you might cause a script to break because a file path is configured differently than your script anticipates. R packages like [here](https://cran.r-project.org/package=here) and [file.path](https://www.rdocumentation.org/packages/base/versions/3.4.3/topics/file.path) allow you to specify file paths is a way that is more operating system independent. See Jenny Bryan's [blog post](https://www.tidyverse.org/articles/2017/12/workflow-vs-script/) for this and other R tips.
<br>
