...
# install the DESeq2 package from bioconductor using BiocManager::install()
# NOTE, this is how you WOULD install it, but since we're using the cloud
# instance, the package is already installed
...
In bioinformatics, there are often many different tools that can be used in a particular instance. The authors of `DESeq2` have [compiled some of them](https://github.com/knausb/DESeq2#software-that-produce-vcf-files). One of those packages that [is available from Bioconductor](https://bioconductor.org/packages/release/bioc/html/VariantAnnotation.html) is called `VariantAnnotation` and may also be of interest to those working with vcf files in R.
...
> - Use that package's `install()` function to install `DESeq2`
